...
/**
 * Switch one agent's wire presentation to PTC Mode (a single `run_code` tool
 * backed by the generated SDK) after promotion. `agent.ctx.tools` is the
 ...
 */
function withWorkspaceLine(assembly, agent, policy) {
  if (!Array.isArray(assembly.sections)) return assembly
  const cwd = agent?.session?.header?.cwd
  const line = typeof cwd === 'string' && cwd.length > 0
    ? `${WORKSPACE_LINE_PREFIX}${cwd}.`
    : ''
  ...
  const baseText = policy.promotedPersonaText || persona.text
  const text = line && !baseText.includes(line) ? `${baseText}${line}` : baseText
  return {
    ...assembly,
...
export function apply(ctx, config) {
  ...
  const promotedPersonaText = optionalString(config.promotedPersonaText, 'promotedPersonaText')
  const policy = {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    if (state.promoted) return withWorkspaceLine(assembled, agent, policy)
